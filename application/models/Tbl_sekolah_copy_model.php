...
class Tbl_sekolah_copy_model extends CI_Model
{
	...
	function json() {
		...
		$this->datatables->from('tbl_sekolah_copy');
        //add this line for join
        //$this->datatables->join('table2', 'tbl_sekolah.field = table2.field');
		$this->datatables->join('tbl_refbentuksekolah', 'tbl_sekolah_copy.id_bentuk_sekolah = tbl_refbentuksekolah.id_bentuk_sekolah');
		$this->datatables->join('regencies', 'tbl_sekolah_copy.regency_id = regencies.regency_id');
		$this->datatables->join('districts', 'tbl_sekolah_copy.id_districts = districts.id_districts');
		$this->datatables->add_column('action', anchor(site_url('sekolahcopy/read/$1'),'<i class="fa fa-eye" aria-hidden="true"></i>', array('class' => 'btn btn-success btn-sm'))." 
			".anchor(site_url('sekolahcopy/update/$1'),'<i class="fa fa-pencil-square-o" aria-hidden="true"></i>', array('class' => 'btn btn-info btn-sm'))." 
			".anchor(site_url('sekolahcopy/delete/$1'),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), 'id_data_sekolah');
		return $this->datatables->generate();
	}

	function jumlahSekolah(){
		$query = $this->$db->query("SELECT * FROM tbl_sekolah_copy");
...
/* End of file Tbl_sekolah_copy_model.php */
/* Location: ./application/models/Tbl_sekolah_copy_model.php */
